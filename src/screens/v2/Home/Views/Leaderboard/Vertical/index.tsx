...
const MAX_WIDTH = scaleSizeW(220)
...
export default () => {
  ...
  const onBoundChange: BoardsListProps['onBoundChange'] = (id) => {
    ...
    requestAnimationFrame(() => { drawer.current?.closeDrawer() })
  }
  ...
  const onSourceChange: HeaderBarProps['onSourceChange'] = (source) => {
    ...
    void getBoardsList(source).then(list => {
      ...
      requestAnimationFrame(() => {
        ...
        requestAnimationFrame(() => { handleBoundChange(source, id) })
      })
    })
  }
    ...
    void getLeaderboardSetting().then(({ source, boardId }) => {
      ...
      void getBoardsList(source).then(list => {
        ...
        if (resolvedId != boardId) {
          void saveLeaderboardSetting({ source, boardId: resolvedId })
        }
  ...
  return (
    <DrawerLayoutFixed
      ...
      style={{ elevation: 1 }}
    >
      <View style={{ width: '100%', flex: 1, flexDirection: 'column', overflow: 'hidden' }}>
